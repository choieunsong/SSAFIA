            ...
            if (state.gameStatus.phase === "DAY_DISCUSSION") {
              ...
              if (message.gameStatus.victim) {
                ...
                if (message.gameStatus.victim === state.playerMe.playerId) {
                  ...
                  if (victimJob) {
                    state.message = `낮의 투표 결과로 인해, <span style="font-size:25px; color:${state.victimColor}">${victimNickname}</span>님이 제거되었습니다.<br/> ${victimNickname}님의 직업은 <span style="font-size:25px; color:${state.victimColor}">마피아</span>이었습니다  곧 밤으로 넘어갑니다.`;
                  } else {
                    state.message = `낮의 투표 결과로 인해, <span style="font-size:25px; color:${state.victimColor}">${victimNickname}</span>님이 제거되었습니다.<br/> ${victimNickname}님의 직업은 <span style="font-size:25px; color:${state.victimColor}">마파아</span>가 아니었습니다  곧 밤으로 넘어갑니다.`;
                  }
